...
const SubInvestor4WalletConBtn: React.FC<Props> = ({ handleOpenModal,setWalletType,walletType,  walletconnection}) => {
...
// export default function ConnectButton({ handleOpenModal,walletConnection }: Props) {
  ...
  function handleConnectWallet() {
    ...
    walletconnection();
    setWalletType(5);
    console.log("current selected Wallet Type : ", 5);
  ...
  return account ? (
    <>
     ...
     : 
        <>
            <StyledStepButton type="button" onClick={handleConnectWallet} className="btn btn-primary mb-1">Attach Sub 4 Wallet</StyledStepButton>
        </>
     }    
    </>
  ) : (
...
export default SubInvestor4WalletConBtn
