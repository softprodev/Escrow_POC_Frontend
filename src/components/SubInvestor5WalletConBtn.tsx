...
const SubInvestor5WalletConBtn: React.FC<Props> = ({ handleOpenModal,setWalletType,walletType,  walletconnection}) => {
...
// export default function ConnectButton({ handleOpenModal,walletConnection }: Props) {
  ...
  function handleConnectWallet() {
    ...
    walletconnection();
    setWalletType(6);
    console.log("current selected Wallet Type : ", 6);
  ...
  return account ? (
    <>
     ...
     : 
        <>
            <StyledStepButton type="button" onClick={handleConnectWallet} className="btn btn-primary mb-1">Attach Sub 5 Wallet</StyledStepButton>
        </>
     }    
    </>
  ) : (
...
export default SubInvestor5WalletConBtn
