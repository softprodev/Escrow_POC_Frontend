...
const SellerWalletConBtn: React.FC<Props> = ({ handleOpenModal,setWalletType,walletType,  walletconnection}) => {
...
// export default function ConnectButton({ handleOpenModal,walletConnection }: Props) {
  ...
  function handleConnectWallet() {
    ...
    walletconnection();
    setWalletType(7);
    console.log("current selected Wallet Type : ", 7);
  ...
  return account ? (
    <>
     ...
     : 
        <>
            <StyledStepButton type="button" onClick={handleConnectWallet} className="btn btn-primary mb-1">Attach Seller Wallet</StyledStepButton>
        </>
     }    
    </>
  ) : (
...
export default SellerWalletConBtn
