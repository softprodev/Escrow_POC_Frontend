...
const SubInvestor2WalletConBtn: React.FC<Props> = ({ handleOpenModal,setWalletType,walletType,  walletconnection}) => {
...
// export default function ConnectButton({ handleOpenModal,walletConnection }: Props) {
  ...
  function handleConnectWallet() {
    ...
    walletconnection();
    setWalletType(3);
    console.log("current selected Wallet Type : ", 3);
  ...
  return account ? (
    <>
     ...
     : 
        <>
            <StyledStepButton type="button" onClick={handleConnectWallet} className="btn btn-primary mb-1">Attach Sub 2 Wallet</StyledStepButton>
        </>
     }    
    </>
  ) : (
...
export default SubInvestor2WalletConBtn
