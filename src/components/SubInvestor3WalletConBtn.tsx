...
const SubInvestor3WalletConBtn: React.FC<Props> = ({ handleOpenModal,setWalletType,walletType,  walletconnection}) => {
...
// export default function ConnectButton({ handleOpenModal,walletConnection }: Props) {
  ...
  function handleConnectWallet() {
    ...
    walletconnection();
    setWalletType(4);
    console.log("current selected Wallet Type : ", 4);
  ...
  return account ? (
    <>
     ...
     : 
        <>
            <StyledStepButton type="button" onClick={handleConnectWallet} className="btn btn-primary mb-1">Attach Sub 3 Wallet</StyledStepButton>
        </>
     }    
    </>
  ) : (
...
export default SubInvestor3WalletConBtn
